<?php
// =============================================
// cloud-unit/index.php
// الصفحة الرئيسية لوحدة الاستضافة والتخزين السحابي
// =============================================
...
require_once BASE_PATH . '/includes/cloud_functions.php';
...
$page_titles = [
    ...
    'servers' => 'إعدادات الخوادم',
    'monitoring' => 'مراقبة التخزين',
    'security' => 'التحديثات الأمنية',
    'reports' => 'التقارير',
    'history' => 'سجل النشاطات',
    'hosting' => 'استضافة مواثع'
];

// رسائل الترحيب
$welcome_messages = [
    'dashboard' => 'مرحباً بك في نظام إدارة الاستضافة السحابي',
    'files' => 'إدارة جميع الملفات والمجلدات في السحابة',
    'deployment' => 'نشر وتحديث التطبيقات والمواقع',
    'backup' => 'إدارة النسخ الاحتياطية وجداول المزامنة',
    'servers' => 'تكوين وإدارة الخوادم',
    'monitoring' => 'مراقبة أداء واستخدام التخزين',
    'security' => 'تطبيق التحديثات الأمنية وإدارة الثغرات',
    'reports' => 'تقارير وإحصائيات النظام',
    'history' => 'سجل النشاطات والتغييرات',
    'hosting' => ' استضافه مواقع '
];
...
try {
    // إجمالي التخزين
    $stmt = $db->query("SELECT COALESCE(SUM(storage_gb), 0) as total, COALESCE(SUM(storage_used_gb), 0) as used FROM cloud_servers");
    ...
    // عدد الخوادم النشطة
    $active_servers = $db->query("SELECT COUNT(*) FROM cloud_servers WHERE status = 'online'")->fetchColumn() ?: 0;
    
    // عدد النسخ الاحتياطية
    $backup_count = $db->query("SELECT COUNT(*) FROM cloud_backups WHERE status = 'completed'")->fetchColumn() ?: 0;
    
    // عدد الملفات اليومية
    $daily_files = $db->query("SELECT COUNT(*) FROM cloud_files WHERE DATE(created_at) = CURDATE() AND is_folder = 0")->fetchColumn() ?: 0;
    
} catch (Exception $e) {
    // إذا حدث خطأ، استخدم القيم الافتراضية
    $total_storage = 2400; // 2.4 TB
    $used_storage = 1560; // 1.56 TB
    $storage_percent = 65;
    $active_servers = 12;
    $backup_count = 245;
    $daily_files = 1248;
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .status-maintenance { background: rgba(59, 130, 246, 0.2); color: #3b82f6; }
        ...
        .file-icon-pdf { background: rgba(239, 68, 68, 0.2); color: #ef4444; }
        .file-icon-image { background: rgba(34, 197, 94, 0.2); color: #22c55e; }
        .file-icon-document { background: rgba(59, 130, 246, 0.2); color: #3b82f6; }
        .file-icon-code { background: rgba(168, 85, 247, 0.2); color: #a855f7; }
        .file-icon-archive { background: rgba(245, 158, 11, 0.2); color: #f59e0b; }
        ...
        .disk-usage {
            ...
            width: 120px;
            border-radius: 50%;
            background: conic-gradient(#3b82f6 0% 75%, #1e293b 75% 100%);
            position: relative;
        }
...
<body class="h-full gradient-bg text-gray-100">
        ...
        <aside class="w-64 bg-slate-900 border-l border-slate-700 flex flex-col">
            ...
            <div class="p-6 border-b border-slate-700">
                <div class="flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">وحدة التخزين السحابي</p>
                    </div>
                </div>
            </div>

            <!-- قائمة التنقل - بنفس تنسيق وحدة التوثيق -->
            <nav class="flex-1 overflow-y-auto scrollbar-custom p-4">
                <div class="space-y-2">
                    ...
                    <a href="?page=files" class="nav-item <?php echo isActive($page, 'files'); ?> w-full text-right px-4 py-3 rounded-lg flex items-center justify-end">
                    ...
                    <a href="?page=deployment" class="nav-item <?php echo isActive($page, 'deployment'); ?> w-full text-right px-4 py-3 rounded-lg flex items-center justify-end">
                        <span>عمليات النشر</span>
                        <svg class="w-5 h-5 mr-3 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="?page=backup" class="nav-item <?php echo isActive($page, 'backup'); ?> w-full text-right px-4 py-3 rounded-lg flex items-center justify-end">
                    ...
                    <a href="?page=servers" class="nav-item <?php echo isActive($page, 'servers'); ?> w-full text-right px-4 py-3 rounded-lg flex items-center justify-end">
                        <span>إعدادات الخوادم</span>
                        <svg class="w-5 h-5 mr-3 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/>
                        </svg>
                    </a>

                    <a href="?page=monitoring" class="nav-item <?php echo isActive($page, 'monitoring'); ?> w-full text-right px-4 py-3 rounded-lg flex items-center justify-end">
            ...
            <header class="bg-slate-900 border-b border-slate-700 px-8 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4 space-x-reverse">
                        <button onclick="uploadFiles()" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-semibold transition-all cyber-glow flex items-center">
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0l-4 4m4-4v12"/>
                            </svg>
                            رفع ملفات
                        </button>
                        <button onclick="refreshStorage()" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm font-semibold transition-all flex items-center">
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            تحديث البيانات
                ...
                <div class="grid grid-cols-4 gap-4 mt-4">
                    <div class="bg-slate-800 rounded-lg p-3 flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-400">إجمالي التخزين</p>
                            <p class="text-lg font-bold text-blue-400"><?php echo round($total_storage / 1000, 1); ?> TB</p>
                        </div>
                    ...
                    <div class="bg-slate-800 rounded-lg p-3 flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-400">خوادم نشطة</p>
                            <p class="text-lg font-bold text-green-400"><?php echo $active_servers; ?></p>
                        </div>
                    ...
                    <div class="bg-slate-800 rounded-lg p-3 flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-400">نسخ احتياطي</p>
                            <p class="text-lg font-bold text-purple-400"><?php echo number_format($backup_count); ?></p>
                        </div>
                    ...
                    <div class="bg-slate-800 rounded-lg p-3 flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-400">ملفات اليوم</p>
                            <p class="text-lg font-bold text-yellow-400"><?php echo number_format($daily_files); ?></p>
                        </div>
            ...
            <form id="upload-form" enctype="multipart/form-data" onsubmit="handleUpload(event)">
                ...
                <div class="space-y-4">
                    ...
                    <div class="upload-area p-8 text-center cursor-pointer"
                        ...
                        <p class="text-gray-400 mb-2">اسحب وأفلت الملفات هنا للرفع</p>
                    ...
                    <!-- اختيار المشروع -->
                    <div>
                        <label class="block text-sm font-semibold mb-2 text-right">المشروع المستهدف</label>
                        <select name="project_id" class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:border-blue-500">
                            <option value="">-- عام (جميع المشاريع) --</option>
                            <?php
                            try {
                                $projects = $db->query("SELECT id, project_name FROM cloud_projects WHERE status = 'active' ORDER BY project_name")->fetchAll();
    ...
    <script>
        ...
        function handleUpload(event) {
            ...
            showNotification(`📤 جاري رفع ${selectedFiles.length} ملف...`, 'info');
        ...
        function refreshStorage() {
            ...
            setTimeout(() => {
                hideLoading();
                showNotification('🔄 تم تحديث بيانات التخزين', 'success');
                location.reload();
            }, 1000);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('✅ وحدة الاستضافة والتخزين السحابي جاهزة');
