 ...
 * @param string $cr رقم السجل التجاري
 * @return bool
 */
function validateCommercialRegister($cr) {
    $pattern = '/^[0-9]{10}$/';
    return preg_match($pattern, $cr);
}
...
/**
 * إضافة إشعار جديد للعميل
 * @param int $client_id العميل
 ...
 * @return bool
 */
function addClientNotification($client_id, $title, $message, $type = 'info', $link = '#') {
    global $db;
    
    $sql = "INSERT INTO client_notifications (client_id, type, title, message, link, created_at) 
            VALUES (?, ?, ?, ?, ?, NOW())";
    
    $stmt = $db->prepare($sql);
    $result = $stmt->execute([$client_id, $type, $title, $message, $link]);
 ...
 * @return array
 */
function uploadClientFile($file, $client_id, $allowed_types = ['pdf', 'docx', 'xlsx', 'jpg', 'png', 'zip']) {
    $response = [
        'success' => false,
        'message' => '',
        'filename' => ''
    ...
    // إنشاء مسار مخصص للعميل
    $target_dir = __DIR__ . "/../../uploads/client/{$client_id}/" . date('Y/m/');
    ...
    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        ...
        $response['filepath'] = "/uploads/client/{$client_id}/" . date('Y/m/') . $new_filename;
    } else {
...
function buildProjectStatusBadge($status) {
    $colors = [
        ...
        'under_study' => 'bg-blue-600',
        'contract_pending' => 'bg-purple-600',
    ...
    return "<span class='px-3 py-1 rounded-full text-xs font-semibold {$color} bg-opacity-20 {$color} text-{$color}'>{$text}</span>";
}

/**
 * إنشاء شارة حالة الفاتورة
 * @param string $status الحالة
 * @return string
 */
function buildInvoiceStatusBadge($status) {
    $colors = [
        'draft' => 'bg-gray-600',
        'sent' => 'bg-blue-600',
        'pending' => 'bg-yellow-600',
